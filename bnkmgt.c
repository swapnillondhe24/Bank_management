...
void fund_transfer()
{
    ...
    if (c==0)
    {
...

        {
            ...
            gotoxy(3,12); printf("Amount to be transferred (in INRs.) : "); scanf("%f",&amt);
...
if (c==13)
{
    ...
    for(i=0;i<strlen(ac.address);i++)
            ...
            if (ac.address[i]=='+') putchar(32);//puts + in place of whitespaces
...
void transaction()
{
...

    {
        ...
        gotoxy(50,6);puts("   Dr. (INRs.)");

        gotoxy(65,6);puts("   Cr. (INRs.)");
...
void admin()
{
    ...
    if(strcmp(uname,"ADMIN")==0&&strcmp(pass,"MICROPRJCT")==0) c=1;//if user name and passpord matches, the value of c is changed from 0 to 1. The purpose of changing value of c is seen @line 77 where the password is asked again if c!=0 i.e. if given password is invalid
...
{
	char a[22]="BANK MANAGEMENT SYSTEM";
    int i;
	...
	gotoxy(33,12);for(i=0;i<13;i++)
	{
	delay(30);
	printf("%c",249);
	}
    gotoxy(33,11);delay(50);printf("Omkar kadlag\n");
	gotoxy(33,14);for(i=0;i<13;i++)
	{
	delay(30);
	printf("%c",249);
	}
    gotoxy(33,13);delay(50); printf("Samarth Misal\n");
    gotoxy(33,16);for(i=0;i<13;i++)
	{
	delay(30);
	printf("%c",249);
	}
	gotoxy(33,15); delay(50);printf("Swapnil Londhe\n");
	{
	delay(30);
	printf("%c",249);
	}
...
void num2word(double n,char word[])
{
    ...
    for(i=0;i<5;i++)
    {
        ...
        if(l!=0)
       {
            switch(i)
            {
                case 0: strcpy(x,"Trillon"); break;

                case 1: strcpy(x,"Billion "); break;
...
void gotoxy (int x, int y)
        ...
        coord.Y = y+9; // X and Y coordinates
...

{
   ...
   char d[30]=" WELCOME TO OUR MICRO-PROJECT";
	char e[34]="Press any key to continue.........";
